...
/*PROBLEM 1*/
/*No foreign key to match them*/
SELECT fname, lname, UPPER(class_description)
FROM student s, class c
WHERE c.class_description = 'Database Programming';
...
/*PROBLEM 3*/
/*Cant join two tables yet*/
SELECT fname, lname FROM student
WHERE fname NOT IN('Jack', 'John', 'Bob') 
AND (SELECT * FROM class WHERE class_description = 'Operating systems');

/*PROBLEM 4*/
SELECT ssn, fname, lname, MONTHS_BETWEEN(SYSDATE - dob/12) AS age
FROM student
WHERE fname = 'J%' 
...
WHERE lname = '%nn%'
AND (SELECT class_description FROM student_class WHERE class_description = '%h%';)
ORDER BY lname;
...
DELETE *
FROM class
WHERE city = 'Sacramento' AND salary > 15000;